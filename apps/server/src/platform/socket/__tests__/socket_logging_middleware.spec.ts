...
describe('wrapSocketHandler', () => {
  it('logs start and success when handler succeeds', async () => {
    ...
    const handler = async (req: { data: string }, ctx: { correlationId: string; socketId: string }) => {
    ...
    expect(logger.infoLogs[0]?.message).toContain('Operation started: test:event');
    expect(logger.infoLogs[1]?.message).toContain('Operation succeeded: test:event');
  ...
  it('logs failure and sends structured error response in callback on error', async () => {
    ...
    expect(logger.errorLogs[0]?.message).toContain('Operation failed: test:fail');
  });
});
