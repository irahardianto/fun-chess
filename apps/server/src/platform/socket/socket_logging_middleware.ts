...
export function wrapSocketHandler<TReq, TRes>(
  ...
  return async (req: TReq, callback?: (res: TRes) => void): Promise<TRes | undefined> => {
    ...
    try {
      ...
      if (typeof callback === 'function') {
        const errorRecord = err as { code?: string; message?: string; details?: Record<string, unknown> };
        const code = (errorRecord?.code as SocketErrorPayload['code']) || 'ERR_INTERNAL_SERVER';
