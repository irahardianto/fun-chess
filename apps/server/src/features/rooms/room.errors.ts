...
export class RoomAlreadyExistsError extends AppError {
  constructor(roomCode: string) {
    super('ERR_ROOM_ALREADY_EXISTS', `Room '${roomCode}' already exists`, 409, { roomCode });
  }
}

export class InvalidRoomCodeError extends AppError {
  constructor(roomCode: string) {
    super('ERR_INVALID_ROOM_CODE', `Invalid room code '${roomCode}'. Code must be 4 uppercase characters`, 400, { roomCode });
  }
}
...
export class GameNotActiveError extends AppError {
  constructor(status: string) {
    super('ERR_GAME_NOT_ACTIVE', `Game is not currently active (current status: ${status})`, 400, { status });
  }
}

export class PlayerNotInRoomError extends AppError {
  constructor(socketId?: string) {
    super('ERR_PLAYER_NOT_IN_ROOM', 'Socket does not belong to an active player in this room', 403, { socketId });
  }
}

export class UnauthorizedError extends AppError {
  constructor(message = 'Unauthorized session token or invalid player credentials') {
...
export class InvalidPayloadError extends AppError {
  constructor(field: string, reason: string) {
    super('ERR_INVALID_PAYLOAD', `Invalid payload: ${field} - ${reason}`, 400, { field, reason });
  }
}
