...
describe('RoomService', () => {
  ...
  describe('createRoom', () => {
    ...
    it('supports random color choice', async () => {
      ...
      const hasOnePlayer = Boolean(room.whitePlayer) !== Boolean(room.blackPlayer);
    ...
    it('rejects player name longer than 20 characters', async () => {
      await expect(
        service.createRoom({ playerName: 'ThisNameIsWayTooLongForChess' }, 'sock_1')
      ).rejects.toThrow(InvalidPayloadError);
    });
  });
  ...
  describe('joinRoom', () => {
    it('allows player 2 to join and transitions room to playing', async () => {
      ...
      const { room: joined, player, sessionToken } = await service.joinRoom(
    ...
    it('rejects joining a full room', async () => {
      ...
      await service.joinRoom({ roomCode: created.roomCode, playerName: 'P2' }, 'sock_2');

      await expect(
        service.joinRoom({ roomCode: created.roomCode, playerName: 'P3' }, 'sock_3')
  ...
  describe('leaveRoom & handleDisconnect', () => {
    it('deletes room when host leaves lobby', async () => {
      ...
      const { shouldDelete } = await service.leaveRoom(created.roomCode, 'sock_1');
  ...
  describe('handleAbandonmentForfeit', () => {
    it('awards win by abandonment to opponent when disconnected player times out', async () => {
      ...
      const result = await service.handleAbandonmentForfeit(created.roomCode, created.hostId);
      ...
      expect(result?.gameOverPayload.message).toContain('WhiteHost disconnected. BlackJoiner won by abandonment!');
    ...
    it('awards win to White if Black player drops and times out', async () => {
      ...
      // Abandonment timeout fires
      const result = await service.handleAbandonmentForfeit(created.roomCode, blackPlayer.id);
      expect(result).not.toBeNull();
    ...
    it('returns null if disconnected player already reconnected', async () => {
      ...
      await service.reconnect(
        { roomCode: created.roomCode, playerId: created.hostId, sessionToken: wToken },
        'sock_w_new'
      );
