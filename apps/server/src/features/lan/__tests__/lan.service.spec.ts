...
describe('LanService', () => {
  ...
  describe('getAllLanInterfaces & getLocalLanIp', () => {
    it('filters out internal loopback and IPv6 interfaces', () => {
      const mockInterfaces: NodeJS.Dict<NetworkInterfaceInfo[]> = {
        ...
        eth0: [
          { address: '192.168.1.150', netmask: '255.255.255.0', family: 'IPv4', mac: '00:11:22:33:44:55', internal: false, cidr: '192.168.1.150/24' },
          { address: 'fe80::1', netmask: 'ffff:ffff:ffff:ffff::', family: 'IPv6', mac: '00:11:22:33:44:55', internal: false, cidr: 'fe80::1/64', scopeid: 1 },
        ],
      };
    ...
    it('prioritizes 192.168.* over 10.* and 172.* subnets', () => {
      const mockInterfaces: NodeJS.Dict<NetworkInterfaceInfo[]> = {
        wlan0: [
          { address: '10.0.0.5', netmask: '255.0.0.0', family: 'IPv4', mac: '00:11:22:33:44:55', internal: false, cidr: '10.0.0.5/8' },
        ],
        eth0: [
          { address: '192.168.1.200', netmask: '255.255.255.0', family: 'IPv4', mac: '00:11:22:33:44:56', internal: false, cidr: '192.168.1.200/24' },
        ],
      };
    ...
    it('falls back to 172.16-31.* if 192.168.* and 10.* are not available', () => {
      const mockInterfaces: NodeJS.Dict<NetworkInterfaceInfo[]> = {
        docker0: [
          { address: '172.20.0.1', netmask: '255.255.0.0', family: 'IPv4', mac: '00:11:22:33:44:55', internal: false, cidr: '172.20.0.1/16' },
        ],
      };
  ...
  describe('QR Code Generation', () => {
    it('generates a valid SVG string', async () => {
      const svg = await service.generateQrCodeSvg('http://192.168.1.100:3000?room=STAR');
      expect(svg).toContain('<svg');
      expect(svg).toContain('</svg>');
    });

    it('generates a valid base64 data URL', async () => {
      const dataUrl = await service.generateQrCodeDataUrl('http://192.168.1.100:3000?room=STAR');
