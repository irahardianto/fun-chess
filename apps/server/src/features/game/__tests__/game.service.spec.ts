...
describe('GameService', () => {
  ...
  const createActiveGameRoom = (code = 'GAME', customFen?: string): RoomState => {
  ...
  describe('makeMove', () => {
    ...
    it('detects checkmate and sets room status to game_over', async () => {
      ...
      const matePositionFen = 'r1bqkb1r/pppp1ppp/2n2n2/4p2Q/2B1P3/8/PPPP1PPP/RNB1K1NR w KQkq - 4 4';
    ...
    it('rejects move if room does not exist', async () => {
      await expect(
        service.makeMove({ roomCode: 'NOPE', move: { from: 'e2', to: 'e4' } }, 'sock_white')
    ...
    it('rejects move if game is not active', async () => {
      ...
      await expect(
        service.makeMove({ roomCode: 'LOBBY', move: { from: 'e2', to: 'e4' } }, 'sock_white')
    ...
    it('rejects move if socket is not a player in room', async () => {
      ...
      await expect(
        service.makeMove({ roomCode: 'TEST', move: { from: 'e2', to: 'e4' } }, 'sock_stranger')
    ...
    it('rejects move when it is not player turn', async () => {
      ...
      await expect(
        service.makeMove({ roomCode: 'TEST', move: { from: 'e7', to: 'e5' } }, 'sock_black')
    ...
    it('rejects illegal chess move', async () => {
      ...
      await expect(
        service.makeMove({ roomCode: 'TEST', move: { from: 'e2', to: 'e7' } }, 'sock_white')
  ...
  describe('resign', () => {
    it('allows white to resign and awards victory to black', async () => {
      ...
      const { room: updatedRoom, gameOverPayload } = await service.resign('TEST', 'sock_white');
    ...
    it('rejects resignation if game is already over', async () => {
      ...
      await expect(service.resign('OVER', 'sock_white')).rejects.toThrow(GameNotActiveError);
    });
  });
  ...
  describe('requestRematch and respondRematch', () => {
    ...
    it('swaps player colors and restarts game when rematch is accepted', async () => {
      ...
      const result = await service.respondRematch('REMATCH', 'sock_black', true);
    ...
    it('reverts room to game_over when rematch is declined', async () => {
      ...
      await service.requestRematch('REMATCH', 'sock_white');
      const result = await service.respondRematch('REMATCH', 'sock_black', false);
