...
class TestSocket {
  ...
  public handlers = new Map<string, (payload: any, callback?: any) => Promise<any>>();
  ...
  async trigger(event: string, payload: unknown, callback?: (res: any) => void) {
...
describe('Game Socket Handlers', () => {
  ...
  const setupActiveRoom = async (code = 'CHSS', customFen?: string): Promise<RoomState> => {
  ...
  describe('game:move', () => {
    ...
    it('emits game:check when move puts opponent in check', async () => {
      ...
      const openCheckFen = 'rnb1kbnr/pppp1ppp/8/8/4q3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 1';
      await setupActiveRoom('CHCK', openCheckFen);

      // White plays Qe2 to block or counter-check
      const checkPositionFen = 'rnbqkbnr/pppp1ppp/8/8/4P3/8/PPPPQPPP/RNB1KBNR w KQkq - 0 1';
      // Let's create a move that delivers check: White plays Qh5+ after 1. e4 e5 2. f4 exf4
      const preCheckFen = 'rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2';
    ...
    it('emits game:over with checkmate reason on winning checkmate move', async () => {
      const scholarMateFen = 'r1bqkb1r/pppp1ppp/2n2n2/4p2Q/2B1P3/8/PPPP1PPP/RNB1K1NR w KQkq - 4 4';
  ...
  describe('game:resign', () => {
    it('concedes game, emits game:over to room with resignation reason and awards win to opponent', async () => {
      ...
      let ackResponse: any;
      await whiteSocket.trigger('game:resign', { roomCode: 'RESIGN' }, (res) => {
        ackResponse = res;
      });
  ...
  describe('game:offer_draw & game:respond_draw', () => {
    it('sends game:draw_offered to opponent socket', async () => {
      ...
      let ackResponse: any;
      await whiteSocket.trigger('game:offer_draw', { roomCode: 'DRAW' }, (res) => {
        ackResponse = res;
      });

      expect(ackResponse.success).toBe(true);

      const drawOfferedEmit = io.toEmits.find((e) => e.event === 'game:draw_offered');
      expect(drawOfferedEmit).toBeDefined();
      expect(drawOfferedEmit?.room).toBe('sock_black');
      expect((drawOfferedEmit?.payload as any).fromPlayerName).toBe('White Player');
    });
    ...
    it('emits game:draw_declined to room when draw is declined', async () => {
      ...
      const declinedEmit = io.toEmits.find((e) => e.event === 'game:draw_declined');
  ...
  describe('game:request_rematch & game:respond_rematch', () => {
    it('broadcasts game:rematch_requested when requested after game over', async () => {
      ...
      let ackResponse: any;
      await whiteSocket.trigger('game:request_rematch', { roomCode: 'REMATCH' }, (res) => {
        ackResponse = res;
      });

      expect(ackResponse.success).toBe(true);

      const rematchReqEmit = io.toEmits.find((e) => e.event === 'game:rematch_requested');
    ...
    it('swaps colors and emits game:rematch_started when opponent accepts rematch', async () => {
      ...
      await whiteSocket.trigger('game:request_rematch', { roomCode: 'REMATCH' }, () => {});
      ...
      const rematchStartedEmit = io.toEmits.find((e) => e.event === 'game:rematch_started');
    ...
    it('emits game:rematch_declined when opponent declines rematch', async () => {
      ...
      const rematchDeclinedEmit = io.toEmits.find((e) => e.event === 'game:rematch_declined');
      expect(rematchDeclinedEmit).toBeDefined();
      expect(rematchDeclinedEmit?.room).toBe('REMATCH');
      expect((rematchDeclinedEmit?.payload as any).byPlayerId).toBe('p_black_id');
    });
  });
});
