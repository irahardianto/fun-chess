...
import type { MoveResult, PieceType, PieceColor } from "@fun-chess/shared";
...
describe("Server ChessEngine Benchmark", () => {
  ...
  for (let i = 0; i < moves.length; i++) {
    ...
    moveHistory.push({
      ...
      captured: res.captured ? (res.captured as PieceType) : undefined,
      flags: res.flags,
      fen: chess.fen(),
      moveNumber: i + 1,
      timestamp: Date.now(),
  ...
  bench("validateAndApplyMove with 20-move history", () => {
    ChessEngine.validateAndApplyMove(
      currentFen,
      legalMove,
      "w",
      moveHistory,
