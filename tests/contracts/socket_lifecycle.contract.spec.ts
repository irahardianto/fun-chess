...
describe('Socket.io Lifecycle Contracts', () => {
  ...
  describe('room:create Contract', () => {
    it('should generate a valid 4-letter uppercase code and set status to lobby when host creates a room', async () => {
      ...
      const response = await emitAck<
        CreateRoomRequest,
        { success: true; room: RoomState; sessionToken: string } | { success: false; error: SocketErrorPayload }
  ...
  describe('room:join Contract', () => {
    it('should assign opposite color, transition room status to playing, and broadcast game:started when joiner connects', async () => {
      ...
      >(hostClient, 'room:create', { playerName: 'Alice', preferredColor: 'w' });
      expect(createRes.success).toBe(true);
      const roomCode = createRes.room.roomCode;

      // Act: Joiner joins room, listen for game:started on both host and joiner
      const hostGameStartPromise = waitForEvent<GameState>(hostClient, 'game:started');
      const joinerGameStartPromise = waitForEvent<GameState>(joinerClient, 'game:started');
      const hostPlayerJoinedPromise = waitForEvent<{ player: Player; room: RoomState }>(
        hostClient,
        'room:player_joined'
      );
      ...
      const joinRes = await emitAck<
        JoinRoomRequest,
        { success: true; room: RoomState; player: Player; sessionToken: string } | { success: false; error: SocketErrorPayload }
  ...
  describe('game:move Contract', () => {
    ...
    beforeEach(async () => {
      ...
      >(joinerClient, 'room:join', { roomCode: activeRoomCode, playerName: 'Bob' });
    });
    ...
    it('should accept legal move, update FEN, and broadcast game:moved when made on correct turn', async () => {
      ...
      // Act
      const hostMovedPromise = waitForEvent<{ move: MoveResult; gameState: GameState }>(
        hostClient,
        'game:moved'
      );
      const joinerMovedPromise = waitForEvent<{ move: MoveResult; gameState: GameState }>(
        joinerClient,
        'game:moved'
      );
