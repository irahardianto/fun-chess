...
describe('Game Moves & Rules Integration Tests', () => {
  ...
  it('should process multi-ply move exchange and maintain accurate move history and turn state', async () => {
    ...
    const moves: { client: TypedSocketClient; from: string; to: string; expectedTurn: PieceColor }[] = [
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      const moveEventPromise = waitForEvent<{ move: MoveResult; gameState: GameState }>(
        whiteClient,
        'game:moved'
      );
  ...
  it('should emit game:check when a move places the opponent king in check', async () => {
    ...
    const checkEventPromise = waitForEvent<{ inCheck: PieceColor; kingSquare: string }>(
      blackClient,
      'game:check'
    );
  ...
  it('should track captured pieces and update material balance when pieces are captured', async () => {
    ...
    const move3Promise = waitForEvent<{ move: MoveResult; gameState: GameState }>(
      whiteClient,
      'game:moved'
    );
