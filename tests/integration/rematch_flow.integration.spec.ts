...
describe('Rematch Flow Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    >(player1Client, 'room:create', { playerName: 'Player1', preferredColor: 'w' });
    ...
    >(player2Client, 'room:join', { roomCode: activeRoomCode, playerName: 'Player2' });

    // Conclude game to enter GAME_OVER state (Player 1 resigns)
    const gameOverPromise = waitForEvent(player2Client, 'game:over');
    player1Client.emit('game:resign', { roomCode: activeRoomCode } as ResignRequest);
  ...
  it('should swap player colors, reset board, and broadcast game:rematch_started when rematch is accepted', async () => {
    ...
    const rematchRequestPromise = waitForEvent<{ requestedBy: string; requesterName: string }>(
      player2Client,
      'game:rematch_requested'
    );

    player1Client.emit('game:request_rematch', { roomCode: activeRoomCode } as RequestRematchRequest);
    const rematchReq = await rematchRequestPromise;
    expect(rematchReq.requesterName).toBe('Player1');

    // Act: Player 2 accepts rematch
    const p1RematchStartPromise = waitForEvent<GameState>(player1Client, 'game:rematch_started');
    const p2RematchStartPromise = waitForEvent<GameState>(player2Client, 'game:rematch_started');
    ...
    const updatedRoom = await serverInstance.roomStore.findByCode(activeRoomCode);
