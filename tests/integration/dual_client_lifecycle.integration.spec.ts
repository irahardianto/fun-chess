...
describe('Dual-Client Full Game Lifecycle Integration Suite', () => {
  ...
  it('should execute a full 2-player match with turn enforcement, check, and checkmate (Scholar\'s Mate)', async () => {
    ...
    >(player1, 'room:create', { playerName: 'Leo (Host)', preferredColor: 'w' });
    ...
    const p1GameStartedPromise = waitForEvent<GameState>(player1, 'game:started');
    const p2GameStartedPromise = waitForEvent<GameState>(player2, 'game:started');
    ...
    async function playMove(
      ...
      const p1Moved = waitForEvent<{ move: MoveResult; gameState: GameState }>(player1, 'game:moved');
      const p2Moved = waitForEvent<{ move: MoveResult; gameState: GameState }>(player2, 'game:moved');
    ...
    const [gameOverP1, gameOverP2] = await Promise.all([p1GameOver, p2GameOver]);
  ...
  it('should execute draw agreement lifecycle', async () => {
    ...
    const drawOfferedPromise = waitForEvent<{ fromPlayerId: string; fromPlayerName: string }>(
      player2,
      'game:draw_offered'
    );
    ...
    player2.emit('game:respond_draw', { roomCode, accept: true } as RespondDrawRequest);
  ...
  it('should handle rematch request with color swap and fresh game start', async () => {
    ...
    const rematchReqPromise = waitForEvent<{ requestedBy: string; requesterName: string }>(
      player2,
    ...
    const p1RematchStarted = waitForEvent<GameState>(player1, 'game:rematch_started');
    const p2RematchStarted = waitForEvent<GameState>(player2, 'game:rematch_started');

    player2.emit('game:respond_rematch', { roomCode, accept: true } as RespondRematchRequest);

    const [p1State, p2State] = await Promise.all([p1RematchStarted, p2RematchStarted]);
    expect(p1State.fen).toBe('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1');
