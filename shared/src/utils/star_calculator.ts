 ...
 * Semantically identical to calculateStars with puzzle-specific parameter nomenclature.
 *
 * @param hintsUsed - Number of hints requested during the attempt
...
export function calculatePuzzleStars(
  ...
  return calculateStars(hintsUsed, mistakesCount);
}

/**
 * Computes accuracy percentage for a completed scenario run or puzzle drill.
 *
 * @param totalStepsOrItems - Total number of steps or items in the run
 * @param mistakesCount - Total incorrect moves played or errors made
 * @returns Accuracy percentage (0 - 100)
 */
export function calculateAccuracy(
  totalStepsOrItems: number,
  mistakesCount: number,
): number {
  const safeTotal = Math.max(1, totalStepsOrItems);
 ...
 *
 * @param solved - Total items solved
 * @param total - Total items attempted
 * @returns Accuracy percentage (0 - 100)
 */
