...
describe('Gamified Puzzle Hub Contracts & Models', () => {
  describe('Taxonomy & Union Types', () => {
    ...
    it('validates PuzzleDifficultyTier calibrations', () => {
      const tiers: PuzzleDifficultyTier[] = ['novice', 'easy', 'medium', 'hard', 'expert'];
    ...
    it('validates 3-tier HintLevel and HintTierName values', () => {
      ...
      const tierNames: HintTierName[] = ['none', 'piece_nudge', 'target_glow', 'full_solution'];
  ...
  describe('Entity & Bundle Models', () => {
    ...
    it('constructs a PuzzleThemeDescriptor for UI rendering', () => {
      const descriptor: PuzzleThemeDescriptor = {
        ...
        kidFriendlyTip: 'Look for pieces standing on the same color diagonals or L-shapes!',
  ...
  describe('Progress Persistence & Store Contracts', () => {
    it('constructs a full PuzzleProgress object with theme mastery and rating profile', () => {
      const ratingProfile: AdaptiveRatingState = {
        ...
        bestStreak: 9,
        ratingHistory: [
          { timestamp: 1700000000000, rating: 1238, puzzleId: 'puz_001', delta: 12 },
          { timestamp: 1700000010000, rating: 1250, puzzleId: 'puz_002', delta: 12 },
        ],
      };
    ...
    it('implements PuzzleProgressStore interface with an in-memory mock', async () => {
      ...
      const mockStore: PuzzleProgressStore = {
        ...
        async recordPuzzleAttempt(puzzleId: string, theme: PuzzleTheme, result: PuzzleAttemptResult, stars: StarRating) {
          ...
          const masteryLevel = newSolvedCount >= 20 ? 'master' : newSolvedCount >= 8 ? 'apprentice' : 'novice';
          ...
          storedProgress = {
            ...
            solvedPuzzles: solved
              ? { ...storedProgress.solvedPuzzles, [puzzleId]: { stars, solvedAt: Date.now() } }
              : storedProgress.solvedPuzzles,
            lastActiveAt: Date.now(),
          };
          return storedProgress;
        },
        async saveArcadeResult(mode: 'puzzle_rush' | 'streak_survivor', score: number, streak: number) {
          storedProgress = {
            ...storedProgress,
            arcadeStats: {
              ...storedProgress.arcadeStats,
              puzzleRushHighScore: mode === 'puzzle_rush' ? Math.max(storedProgress.arcadeStats.puzzleRushHighScore, score) : storedProgress.arcadeStats.puzzleRushHighScore,
              puzzleRushBestStreak: mode === 'puzzle_rush' ? Math.max(storedProgress.arcadeStats.puzzleRushBestStreak, streak) : storedProgress.arcadeStats.puzzleRushBestStreak,
              streakSurvivorHighScore: mode === 'streak_survivor' ? Math.max(storedProgress.arcadeStats.streakSurvivorHighScore, score) : storedProgress.arcadeStats.streakSurvivorHighScore,
              totalRushRuns: mode === 'puzzle_rush' ? storedProgress.arcadeStats.totalRushRuns + 1 : storedProgress.arcadeStats.totalRushRuns,
            },
            lastActiveAt: Date.now(),
          };
          return storedProgress;
      ...
      const afterSolve = await mockStore.recordPuzzleAttempt('puz_fork_001', 'fork', 'solved_first_try', 3);
  ...
  describe('Engine & Rules Calculation Contracts', () => {
    it('validates PlayerMoveAction and MoveValidationOutcome', () => {
      ...
      const outcome: MoveValidationOutcome = {
        ...
        nextFen: 'r1bqkb1r/pppp1Qpp/2n5/4p3/2B1n3/8/PPPP1PPP/RNB1K1NR b KQkq - 0 4',
        nextMoveIndex: 1,
    ...
    it('implements pure AdaptiveRatingCalculator contract', () => {
      const calculator: AdaptiveRatingCalculator = {
        calculateAdjustment(params: RatingAdjustmentParams): RatingAdjustmentResult {
          ...
          let streakBonus = 0;
          if (params.isSuccess && params.hintsUsed === 0 && params.currentStreak >= 3) {
    ...
    it('implements pure PuzzleRushRules contract', () => {
      const rushRules: PuzzleRushRules = {
        ...
        calculateTimeTick(currentSeconds: number, deltaSeconds: number): RushTickResult {
          const timeRemainingSeconds = Math.max(0, currentSeconds - deltaSeconds);
          return {
