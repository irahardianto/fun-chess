...
describe('Expanded Academy & Scenario Curriculum Contracts', () => {
  describe('Taxonomy & Difficulty Tiers', () => {
    ...
    it('validates ScenarioDifficulty calibration including master tier', () => {
      const difficulties: ScenarioDifficulty[] = ['beginner', 'intermediate', 'advanced', 'master'];
  ...
  describe('Scenario Curriculum Entities', () => {
    it('constructs an intermediate tactics scenario (Deflection / Decoy)', () => {
      const scenario: ChessScenario = {
        ...
        icon: '🎯',
        description: 'Learn how to deflect a key defender so you can deliver a decisive blow.',
        estimatedMinutes: 4,
        steps: [
          {
            id: 'step-1',
            ...
            conceptExplanation: 'When the King captures your Rook, the Queen on e7 will lose its defender.',
            ...
            explanationOnSuccess: 'Great deflection! The Queen on e7 is now completely unprotected!',
          },
          {
            id: 'step-2',
            stepNumber: 2,
            ...
            explanationOnSuccess: 'Brilliant combination! You won the Queen and the match! ⭐⭐⭐',
          },
        ],
      };
      ...
      expect(scenario.steps[0]?.opponentResponse?.dialogue).toContain('King had to take');
    });
    ...
    it('constructs a checkmate family scenario (Anastasia Mate)', () => {
      const scenario: ChessScenario = {
        ...
        subtitle: 'Knight and Rook teamwork traps the enemy King against the edge of the board.',
        category: 'checkmate_families',
        difficulty: 'advanced',
        targetAgeGroup: '11-15',
        icon: '👑',
        description: "Experience the famous Anastasia's mate pattern using a Knight to seal escape squares and a Rook on an open file.",
        estimatedMinutes: 5,
        steps: [
          {
            id: 'step-1',
            stepNumber: 1,
            instruction: 'Jump your Knight to e7 to deliver check and seal g8 and g6!',
    ...
    it('constructs an endgame conversion scenario (Lucena Position)', () => {
      const scenario: ChessScenario = {
        ...
        subtitle: 'Learn the fundamental winning technique in Rook and pawn endgames.',
        category: 'endgame_conversions',
        difficulty: 'master',
        targetAgeGroup: '11-15',
        icon: '🌉',
        description: 'The Lucena Position is the most important technical endgame in chess. Learn how to build a bridge with your Rook to promote your pawn safely.',
        estimatedMinutes: 6,
        steps: [
          {
            id: 'step-1',
            stepNumber: 1,
            instruction: 'Move your Rook to the 4th rank (f4) to prepare the bridge!',
            hint: 'Place your Rook on f4.',
            setupFen: '1K1R4/1P6/8/8/5k2/8/8/5r2 w - - 0 1',
            highlightSquares: ['d8', 'd4'],
            playerColor: 'w',
            allowedMoves: [{ from: 'd8', to: 'd4' }],
            explanationOnSuccess: 'Rook is stationed on the 4th rank ready to shield the King!',
          },
        ],
      };
    ...
    it('constructs an opening trap scenario (Legal’s Trap)', () => {
      const scenario: ChessScenario = {
        ...
        targetAgeGroup: '7-10',
        icon: '🎭',
        description: "Legal's Trap is one of the most famous checkmate traps in chess history, sacrificing the Queen for a forced checkmate.",
        estimatedMinutes: 4,
        steps: [
          {
            id: 'step-1',
            stepNumber: 1,
            instruction: 'Jump your Knight from f3 to e5, leaving your Queen on d1 "unprotected"!',
            hint: 'Move your Knight to e5.',
            setupFen: 'r1bqk2r/pppp1ppp/2n5/4p3/2B1n3/3P1N2/PPP2PPP/RNBQK2R w KQkq - 0 5',
            ...
            explanationOnSuccess: 'Black took the bait and took the Queen! Now finish the mate!',
          },
        ],
      };
    ...
    it('constructs a CurriculumSection grouping scenarios', () => {
      const section: CurriculumSection = {
        ...
        subtitle: 'Master the classic mating nets: Anastasia, Arabian, Hook, and Vukovic!',
        icon: '👑',
        scenarios: [],
      };
  ...
  describe('Scenario Progress Store Contract', () => {
    it('implements ScenarioProgressStore with in-memory map adapter', async () => {
      ...
      const testStore: ScenarioProgressStore = {
        ...
        async getProgress(scenarioId: string): Promise<ScenarioProgress | null> {
          return storeMap[scenarioId] ?? null;
        },
        async saveProgress(scenarioId: string, stars: StarRating, hintsUsed: number): Promise<ScenarioProgress> {
