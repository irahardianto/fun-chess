...
describe('Shared Contracts & Data Model Specification', () => {
  describe('Chess Core & Multiplayer Models', () => {
    ...
    it('validates GameOverPayload and GameOverReason union values', () => {
      ...
      const gameOver: GameOverPayload = {
        ...
        finalFen: 'r1bqkb1r/pppp1Qpp/2n5/4p3/2B1n3/8/PPPP1PPP/RNB1K1NR b KQkq - 0 4',
        totalMoves: 7,
        durationSeconds: 45,
      };
  ...
  describe('Academy & Scenario Contracts', () => {
    ...
    it('constructs a multi-step ChessScenario conforming to TutorialStep contract', () => {
      const scenario: ChessScenario = {
        ...
        targetAgeGroup: '7-10',
        icon: '♞',
        description: 'Learn how the Knight jumps in an L-shape to attack two pieces at once!',
        estimatedMinutes: 3,
        steps: [
          {
            id: 'step-1',
            stepNumber: 1,
            instruction: 'Jump your Knight to c7 to deliver check and attack the Rook!',
            conceptExplanation: 'Knights are the only pieces that can jump over other pieces.',
          ...
          },
          {
            ...
            explanationOnSuccess: 'You captured the Rook and won the lesson! ⭐⭐⭐',
          },
        ],
      };
      ...
      expect(scenario.steps[0]?.opponentResponse?.dialogue).toBe('My King must run away!');
    });
  ...
  describe('Single-Player AI & Mascot Persona Contracts', () => {
    it('validates 4 calibrated AI difficulty tiers and mascot IDs', () => {
      const levels: AiDifficultyLevel[] = ['novice', 'beginner', 'intermediate', 'club'];
    ...
    it('constructs a full MascotPersona complying with character specifications', () => {
      const peanutPup: MascotPersona = {
        ...
        eloEstimate: 400,
        difficulty: 'novice',
        description: 'Peanut loves wagging his tail and jumping pawns forward! Makes lots of silly mistakes.',
        themeColor: '#ff9800',
        dialogues: {
  ...
  describe('Shell Navigation & Audio Contracts', () => {
    it('validates AppGameMode options and launch configurations', () => {
      const modes: AppGameMode[] = ['lobby', 'multiplayer_lan', 'solo_ai', 'academy'];
